...
resource "helm_release" "efs_csi_driver" {
  ...
  depends_on = [aws_efs_mount_target.zone_a, aws_efs_mount_target.zone_b]
}

#Optionally since we already init helm provider(just to make it self contained)
...
provider "kubernetes" {
  ...
  token                  = data.aws_eks_cluster.eks_v2.token
